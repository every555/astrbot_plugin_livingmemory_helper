(() => {
  ...
  /* ── CSS helpers ───────────────────────────────────────────── */
  function isDark() {
    // helper 页面为纯深色主题（CSS 写死 #0d0e15），不支持 light，始终用 dark 渲染
    return true;
  }
  ...
  Interaction.prototype._onTouchStart = function(e) {
    ...
    if (e.touches.length === 1) {
      // preventDefault 已在外层 touchstart 调用，这里补空函数避免 _onMouseDown 报 TypeError
  ...
  Interaction.prototype._onTouchMove = function(e) {
    ...
    if (e.touches.length === 1) {
      this._onMouseMove({ clientX: e.touches[0].clientX, clientY: e.touches[0].clientY, preventDefault: function(){} });
    }
    e.preventDefault();
  };
  ...
  Interaction.prototype._onTouchEnd = function(e) {
    ...
    this._onMouseUp({ clientX: t.clientX || 0, clientY: t.clientY || 0, preventDefault: function(){} });
  };
  ...
  Graph2D.prototype.init = function(containerEl, callbacks) {
    ...
    if (typeof window.MutationObserver === "function") {
      this._themeObserver = new MutationObserver(function() { self.animator.wake(); });
  ...
  Graph2D.prototype.focusNode = function(labelOrId) {
    /* 按标签文本或 id 聚焦节点（前端 focusNode 按钮调用） */
    if (!this._nodes || !this._nodes.length) return;
    var key = String(labelOrId == null ? "" : labelOrId).trim().toLowerCase();
    if (!key) return;
    var target = null;
    if (/^\d+$/.test(key)) {
      var nid = Number(key);
      target = this._nodes.filter(function(n) { return n.id === nid; })[0];
    }
    if (!target) {
      target = this._nodes.filter(function(n) {
        return String(n.label || "").toLowerCase() === key;
      })[0];
    }
    if (!target) {
      target = this._nodes.filter(function(n) {
        return String(n.label || "").toLowerCase().indexOf(key) !== -1;
      })[0];
    }
    if (target) this.selectNode(target.id);
  };

  Graph2D.prototype.resetView = function() {
    /* 复位视图：清除选中并重新以最高权重节点为中心布局 */
    this.clearSelection();
  };
  ...
  /* ━━━ 公开 API：运行时调整力导向参数 ━━━ */
  window.Graph2D.setConfig = function(opts) {
    if (opts.repulsion != null) CFG.FORCE_REPULSION = opts.repulsion;
    if (opts.linkDistance != null) CFG.FORCE_LINK_DISTANCE = opts.linkDistance;
    if (opts.gravity != null) CFG.FORCE_GRAVITY = opts.gravity;
  };
})();
